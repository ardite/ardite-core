...
pub fn discover_driver(config: &DriverConfig) -> Result<Box<Driver>, Error> {
...
fn connect_mongodb_driver(_: &DriverConfig) -> Result<Box<Driver>, Error> {
  use driver::mongodb::MongoDriver;
  MongoDriver::connect(config.url()).map(Box::new)
}
